...
import com.sensorguard.app.crypto.KeychainEntry
import com.sensorguard.app.crypto.KeychainStore
import com.sensorguard.app.store.EventRow
import com.sensorguard.app.store.EventSink
...
private fun KeychainEntry.toEntity() = KeychainEntity(
    ...
    retiredAtMs = retiredAtMs, status = when (status) {
        com.sensorguard.app.crypto.DekStatus.ACTIVE -> 0
        com.sensorguard.app.crypto.DekStatus.RETIRED -> 1
        com.sensorguard.app.crypto.DekStatus.DESTROYED -> 2
    },
)
...
private fun KeychainEntity.toEntry() = KeychainEntry(
    id = id, wrappedDek = wrappedDek, createdAtMs = createdAtMs,
    retiredAtMs = retiredAtMs, status = when (status) {
        0 -> com.sensorguard.app.crypto.DekStatus.ACTIVE
        1 -> com.sensorguard.app.crypto.DekStatus.RETIRED
        else -> com.sensorguard.app.crypto.DekStatus.DESTROYED
    },
)