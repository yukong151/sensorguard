...
- **项目**：边缘计算及传感器安全的轻量级 APP（com.yuexiao12.sensorguard）
...
| 进程存活 | 结束时 `shizuku_server`(17620)、`com.yuexiao12.sensorguard:shizuku`(26464)、主进程均存活 |
...
| 10550 | **本安全 App `com.yuexiao12.sensorguard`** | （探测方，非被探测客户端） | 仅做 `dumpsys` 读取，自身不注册监听 |

> 本安全 App 包名 `com.yuexiao12.sensorguard`（uid 10550）是**检测者**，不是被检测的传感器客户端。
